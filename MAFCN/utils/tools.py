...
def test_mIoU(f, model, data_loader, epoch, n_class, print_per_batches=100):
    model.eval()
    num_classes = n_class
    num_batches = len(data_loader)
    hist = np.zeros((num_classes, num_classes))
    with open('./dataset/all_info.json', 'r') as fp:
        info = json.load(fp)
    ...
    for ind, data in enumerate(data_loader):
        ...
        if len(label.flatten()) != len(pred.flatten()):
            print('Skipping: len(gt) = {:d}, len(pred) = {:d}, {:d}'.format(len(label.flatten()), len(pred.flatten()),
                                                                            ind))
            continue
    ...
    f.write('\n epoch %d ===> OA: ' % (epoch) + str(round(np.nanmean(acc) * 100, 2)) + '\n')
    f.flush()
    print('epoch %d ===> mIoU: ' % (epoch) + str(round(np.nanmean(mIoUs) * 100, 2)))
    print(('epoch %d ===> OA: ' % (epoch) + str(round(np.nanmean(acc) * 100, 2))))
    return np.nanmean(mIoUs)


def test_mIoU_MAFCN(f, model, data_loader, epoch, n_class, print_per_batches=100):
    model.eval()
    num_classes = n_class
    num_batches = len(data_loader)
    hist = np.zeros((num_classes, num_classes))
    ...
    for ind, data in enumerate(data_loader):
        image, label = data[0].cuda(), data[1].squeeze()
        # image, label = data['image'].cuda(), data['label'].squeeze()
        out1,_,_,_,_ = model(image)
...
def untransform_label(output):
    output = output.detach().cpu().numpy()
    # output = output.transpose(1, 2, 0)
    output = np.asarray(output*255.0, dtype=np.uint8)
    output = Image.fromarray(output)
    return output

def untransform_image(output):
    style_out = output.detach().mul(255).permute(1, 2, 0).to('cpu', torch.uint8).numpy()
    style_out = Image.fromarray(style_out)
    return style_out

def untransform_image1(output):
    style_out = output.detach().permute(1, 2, 0).to('cpu').numpy()
    style_out = np.uint8(np.clip(style_out, 0.0, 255.0))
...
if __name__ == '__main__':
    with open('../dataset/wh_info.json', 'r') as fp:
        info = json.load(fp)
    name_classes = np.array(info['label'], dtype=np.str)
    num_classes = 8
    for ind_class in range(num_classes):
