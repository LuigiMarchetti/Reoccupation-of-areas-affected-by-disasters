...
def get_arguments():
    ...
    parser.add_argument("--data_dir", type=str,
                        default='/media/disk3/lmy/dg/dataset/smallall/train/',
                        help="dataset path.")
    parser.add_argument("--data_dir_val", type=str, default='/media/disk3/lmy/dg/dataset/smallall/val/',
    ...
    # result
    parser.add_argument("--snapshot_dir", type=str, default='/media/disk3/lmy/dg/MAFCN/smallall/Snap_MAFCN/',
...
def main():
    ...
    # Create network
    net = NetFB(class_num=args.num_classes)
    ...
    loss_hist = np.zeros((num_steps, 9))
    index_i = -1
    OA_hist = 0.2
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_index, data in enumerate(loader):
            ...
            label = label.cuda()
            # print(label.shape)
            out1, out2, out3, out4, out5 = net(images)
            # Segmentation Loss
            # cls_loss_value = loss_calc(output, label)
            cls_loss_value1 = loss_calc(out1, label)
            cls_loss_value2 = loss_calc(out2, label)
            cls_loss_value3 = loss_calc(out3, label)
            cls_loss_value4 = loss_calc(out4, label)
            cls_loss_value5 = loss_calc(out5, label)

            cls_loss_value = cls_loss_value1*0.5 + (cls_loss_value2+cls_loss_value3+cls_loss_value4+cls_loss_value5)*0.25
            _, predict_labels = torch.max(out1, 1)
            ...
            loss_hist[index_i, 1] = cls_loss_value1.item()
            loss_hist[index_i, 2] = cls_loss_value2.item()
            loss_hist[index_i, 3] = cls_loss_value3.item()
            loss_hist[index_i, 4] = cls_loss_value4.item()
            loss_hist[index_i, 5] = cls_loss_value5.item()
            loss_hist[index_i, 6] = cls_loss_value.item()
            loss_hist[index_i, 7] = miu[1]
            loss_hist[index_i, 8] = miu[0]

            optimizer.step()
            batch_time = time.time() - tem_time

            if (batch_index + 1) % 10 == 0:
                print('epoch %d/%d:  %d/%d time: %.6f miu = %.2f acc = %.2f cls_loss1 = %.6f cls_loss2 = %.6f cls_loss3 = %.6f cls_loss4 = %.6f cls_loss5 = %.6f cls_loss = %.6f\n' % (
                epoch + 1, args.num_epoch, batch_index + 1, num_batches, batch_time,
                np.mean(loss_hist[index_i - 9:index_i + 1, 7]) * 100,np.mean(loss_hist[index_i - 9:index_i + 1, 8]) * 100,
                ...
                np.mean(loss_hist[index_i - 9:index_i + 1, 5]),np.mean(loss_hist[index_i - 9:index_i + 1, 6])))
                f.write('epoch %d/%d:  %d/%d time: %.6f miu = %.2f acc = %.2f cls_loss1 = %.6f cls_loss2 = %.6f cls_loss3 = %.6f cls_loss4 = %.6f cls_loss5 = %.6f cls_loss = %.6f\n' % (
                epoch + 1, args.num_epoch, batch_index + 1, num_batches, batch_time,
                np.mean(loss_hist[index_i - 9:index_i + 1, 7]) * 100,np.mean(loss_hist[index_i - 9:index_i + 1, 8]) * 100,
                np.mean(loss_hist[index_i - 9:index_i + 1, 1]),np.mean(loss_hist[index_i - 9:index_i + 1, 2]),
                np.mean(loss_hist[index_i - 9:index_i + 1, 3]),np.mean(loss_hist[index_i - 9:index_i + 1, 4]),
            ...
            if (index_i + 1) % 500 == 0:
                ...
                pred.save('%s/iter%012d_%s_pred.tif' % (pred_img_path, index_i + 1, im_name[0].split('.')[0]))
                true.save('%s/iter%012d_%s_true.tif' % (pred_img_path, index_i + 1, im_name[0].split('.')[0]))
        ...
        OA_new = test_mIoU_MAFCN(f, net, val_loader, epoch + 1,n_class=args.num_classes)
